...
Kindly Note:
Backup of AB directory & Registry just in case, before doing any testing.
Gmail will not allow sharing DLL so it is only via link.WsRTD.dll - Google Drive
Supported Base Interval is 1-Min. This is to ensure Client APP is capable of building bars.But RTD can be sent every second. The fields should be compliant as mentioned in Docs.
The sample_server.py source code is on github.Rtd_Ws_AB_plugin/Samples/Server/Python at main · ideepcoder/Rtd_Ws_AB_plugin · GitHub
ManualGitHub - ideepcoder/Rtd_Ws_AB_plugin: AmiBroker data plugin for Websocket based Json RTD communication
You just need to add a new symbol and see automatic data streaming in chart and RT quote window.Sample server will auto-generate few symbols SYM1... to SYM5, which we add using theConfigure > RETRIEVE button as per AB design.First we run the server, then connect AB else it will complain server not found. ofc, we can reconnect from plugin menu.
And to view logging, Use DebugView we just add a filter "DBG" so all the data is visible.This is mentioned in the documentation file.

Just for confirmation. For production/testing real time purpose,
I need to initiate/run the relay_Server from relay.py and
Run client.py from samples folder (no update required here as wrapper class will be writing the candles) and then
I need to use the vendor_class_wrapper.py template provided in the samples folder to connect to the brokers apis/python library.
If this is correct, I wil try to contribute to vendor class wrapper for my broker.

kindly post this question in the forum so others can reference my answer.
This is not plug-n-play scripts.
DLL in AB plugin folder.
Relay.py is ready to use.
Client.py is an example of fully working fake data generator.
vendor_class_wrapper is a sample class that you can use and implement in client.py.

sure. Just posted it, waiting for approval.
Thank you for your input.
So I will pick up the vendor_class_wrapper and combine that with needed functions/methods from client.py and implement my broker methods and use it as a source for my relay server so it can starting feeding AB.

thats right. currently, the plugin has minimum base time nterval of 1min.This is to ensure client-app knows how to build bars like in vendor_class_wrapper.py
but the data like in the dataframe can be snapshot every second and sent to plugin(RTD).

Yea so implementing my vendor_class_wrapper, I need to make sure I build 1 min bars and then send it to relay.
One dumb question. Suppose if there is a strategy that is built to check for signals in 1 min candles for example, this wont create any delay right. As I need to build the 1min candle in wrapper and then passing to relay and then to AB.
I know you have mentioned in your comment in forum that it can accommodate tick data as well.

nsm51
Feb 1
there is no delay, as and when data arrives from remote source, you snapshot those ticks stored in dataframe in json rtd array and send that to relay.
Run the sample_server.py, it is fully working version with fake data.

pdtrader
Feb 1
super. I will run and check the fake data so I can understand it deeper and develop my broker wrapper class.
Thank you in tons.
Hi,
I am back.
...
bfsym (Backfill)
Is there any other coms our dll plugin can do? I know you are working on a full scale system but just wanted to see if this in pipeline or would be a bad idea to add new some more additional commands which can be used for other purposes.
On additional note, I am working on my vendor wrapper and able to see data in AB for multiple symbols. Working on backfil now. Once done will share it with you for review.

nsm51
Feb 4
Hi,
The thing is this plugin follows AB philosophy, and Tomasz approves the source.
So you can share your feedback, and even such questions can be posted in open thread because then others wont repeat them 
If you have something in mind, I am adding it to the feature list if it is compliant.
I'm working on BrokerWS right now and it is a pain. not c++, the json standard because every broker has a myopic implementation.
